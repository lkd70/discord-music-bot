...
class Hlna {
  ...
  autocomplete(
    @SlashOption("category", {
      autocomplete: function (this: Hlna, interaction: AutocompleteInteraction) {
        interaction.respond([
    ...
    interaction: CommandInteraction | AutocompleteInteraction): void {
    ...
    if (interaction.isAutocomplete()) {
      ...
      if (focusedOption.name === "item") {
        ...
        if (category === "emotes") {
          resp = hlna.emotes.map((item) => ({ name: item.name, value: item.value }))
        } else if (category === "common") {
          resp = hlna.common.map((item) => ({ name: item.name, value: item.value }))
        } else if (category === "consumables") {
          resp = hlna.consumables.map((item) => ({ name: item.name, value: item.value }))
        } else if (category === "other") {
          resp = hlna.other.map((item) => ({ name: item.name, value: item.value }))
        }
        interaction.respond(resp);
      }
    } else {
      // interaction.reply(`${category} ${item} ${count}`);
      const cata = (category === "emotes") ? 
      hlna.emotes : (category === "common") ? 
      hlna.common : (category === "consumables") ? 
      hlna.consumables : hlna.other;
      ...
      if (loc_item) {
        ...
        const reply = `${count.toLocaleString()} ${loc_item.name} will cost ` + 
          `${hexagons.toLocaleString()} Hexagons. ` +
          `\nThat's ${(Math.ceil(count/loc_item.count)).toLocaleString()} purchases from HLNA.`;
        
        const embed = new MessageEmbed()
        .setTitle(`${count.toLocaleString()} ${loc_item.name} = ${hexagons.toLocaleString()} Hexagons`)
